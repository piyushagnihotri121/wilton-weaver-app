...
def search_designs_by_colors(df: pd.DataFrame, selected_colors: List[str], match_type: str) -> pd.DataFrame:
    ...
    if df.empty or not selected_colors:
        return pd.DataFrame()
    ...
    for idx, row in df.iterrows():
        ...
        # Extract all colors from color-related columns
...
def get_available_colors(df: pd.DataFrame) -> List[str]:
    ...
    # Default aviation colors if none found
    if not all_colors:
        all_colors = {
...
with st.sidebar:
    ...
    if st.session_state.search_history:
        ...
        for search in st.session_state.search_history[-3:]:  # Show last 3 searches
...
with col1:
    # Get unique colors from your data (assuming you have a 'Colors' or similar column)
    available_colors = []
    ...
    # If no color columns found, provide common aviation carpet colors
...
if (selected_colors and color_search_button) or (selected_colors and auto_refresh):
    if 'design_df' in st.session_state:
        with st.spinner('🎨 Searching designs by color combinations...'):
            # Find color-related columns in your dataframe
            color_columns = [col for col in st.session_state.design_df.columns if 
                            any(word in col.lower() for word in ['color', 'colour', 'shade', 'dye'])]
            ...
            if color_columns:
                ...
                for _, row in st.session_state.design_df.iterrows():
                    row_colors = []
                    # Extract colors from all color-related columns
                    for col in color_columns:
                        if pd.notna(row[col]):
                    ...
                    # Check match based on selected match type
                    if match_type == "All Colors (AND)":
                ...
                # Get corresponding yarn matches if available
                yarn_color_matches = pd.DataFrame()
                if 'yarn_df' in st.session_state and not design_color_matches.empty and 'Design Name' in design_color_matches.columns:
                ...
                if not design_color_matches.empty:
                    ...
                    # Enhanced metrics for color search
                    col1, col2, col3, col4 = st.columns(4)
                    ...
                    # Create tabs for color search results
                    if not yarn_color_matches.empty:
                        ...
                        with tab3:
                            ...
                            if show_export:
                                # Export functionality for color search
                                buffer = io.BytesIO()
                                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                                ...
                                color_string = "_".join(selected_colors[:3])  # Use first 3 colors for filename
                                st.download_button(
                        ...
                        # Only design matches, no yarn data
                        st.markdown('<div class="dataframe-container">', unsafe_allow_html=True)
                        st.subheader("🎨 Designs Matching Your Color Selection")
                        st.markdown(f"*Found {len(design_color_matches)} designs using {match_info.lower()} criteria*")
                        st.dataframe(design_color_matches, use_container_width=True, height=400)
