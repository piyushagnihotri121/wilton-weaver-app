...
st.markdown("""
<style>
/* ... (CSS omitted for brevity, use the same as in your prompt) ... */
</style>
...
def reduce_even_repeats(color_sequence: str) -> str:
    """
    If all colors in the sequence are repeated an even number of times,
    divide their count by 2 and return the reduced sequence.
    ...
    # Check if all counts are even and > 1
    if all(v % 2 == 0 and v > 1 for v in counts.values()):
        reduced = []
        for color in counts:
            reduced.extend([color] * (counts[color] // 2))
        # Preserve order as first appearance
        seen = set()
        ordered = []
        for color in colors:
            if color not in seen:
                ...
                seen.add(color)
        # Build reduced sequence in order
        result = []
        for color in ordered:
            if color in counts:
                result.extend([color] * (counts[color] // 2))
        return " - ".join(result)
    else:
        # For cases like MINT - MINT - COTTON WHITE - COTTON WHITE - COTTON WHITE - COTTON WHITE
        # If some colors are repeated even times, some not, reduce only those with even counts > 1
        # But as per your example, you want to keep odd-count colors as is, and reduce even-count colors
        # So, for each color, if count > 1 and even, divide by 2, else keep as is
        reduced = []
        ...
        for color in ordered:
            ...
            if cnt > 1 and cnt % 2 == 0:
                reduced.extend([color] * (cnt // 2))
            else:
                reduced.extend([color] * (cnt if cnt == 1 else (cnt if cnt % 2 != 0 else cnt // 2)))
        return " - ".join(reduced)

def get_frame_wise_colors(design_df: pd.DataFrame, yarn_df: pd.DataFrame) -> pd.DataFrame:
    """Create frame-wise color mapping from design and yarn data, with reduced even repeats"""
    ...
    # Deduplicate consecutive colors
    yarn_df['Frame-wise Color'] = yarn_df['Extracted_Colors'].apply(deduplicate_consecutive_colors)
    
    # Reduce even repeats as per requirement
    yarn_df['Frame-wise Color_x'] = yarn_df['Frame-wise Color'].apply(reduce_even_repeats)
    ...
    frame_colors = yarn_df.groupby('Design Name')['Frame-wise Color_x'].apply(
        lambda x: " - ".join([y for y in x if pd.notna(y) and y])
    ).reset_index()
    
    frame_colors.columns = ['Design Name', 'Frame-wise Color_x']
...
if design_file is not None and yarn_file is not None:
    try:
        with st.spinner('🔄 Processing Aviation Carpet Database...'):
            ...
            # Generate frame-wise colors mapping (with _x column)
...
# --- Multi-Filter Search (Color, Construction, No. of Frames, Weft Heads) ---
if (selected_colors or (selected_construction and selected_construction != "Any") or 
    (selected_frames and selected_frames != "Any") or 
    (selected_weft and selected_weft != "Any")) and (color_search_button or auto_refresh):
    if st.session_state.design_df is not None:
        df = st.session_state.design_df
        with st.spinner('🎨 Searching designs by selected filters...'):
            ...
            for idx, row in df.iterrows():
                ...
                # --- Weft Heads Matching ---
                weft_pass = True
                if selected_weft and selected_weft != "Any" and weft_col:
                    weft_pass = str(row[weft_col]).strip() == selected_weft
                # --- Final Decision ---
                if color_pass and construction_pass and frames_pass and weft_pass:
                    matches.append(idx)
            ...
            if not filtered_df.empty:
                ...
                # Display frame-wise colors if available
                if 'Frame-wise Color_x' in filtered_df.columns:
                    st.subheader("Frame-wise Color Mapping (Reduced Even Repeats)")
                    frame_colors_display = filtered_df[['Design Name', 'Frame-wise Color_x']].copy()
                    frame_colors_display = frame_colors_display[frame_colors_display['Frame-wise Color_x'].notna()]
                    if not frame_colors_display.empty:
                        st.dataframe(frame_colors_display, use_container_width=True)
                    else:
                        st.info("No frame-wise color information available for these designs")
